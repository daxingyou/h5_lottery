...
function srip2tInit () {
    ...
    if (sriptSetInterval1) {
        clearInterval(sriptSetInterval1);
    }
...
function moneyFormat (num) {
    ...
    var data = num.split('.');
    data[0] = data[0].toString().replace(/[^\d]/g, '')
        .substr(0, 15);
...
function countUnreadMsgs () {
    ...
    $msgContainer.each(function (index, value) {
        ...
        if (_len > 0) {
            if (index === 0) {
                lastType = $(value).find('#xxm-newmsg')
                    .text()
                    .replace(/^\s|\s$/g, '')
                    .match(/\【(.*)\】/)[1];
                _time = $(value).find('#xxm-time')
                    .text();
            } else {
                _time = $(value).find('dd')
                    .eq(0)
