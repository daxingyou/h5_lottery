		...
		*/
        var htmltext  = '<span id="lt_margin_html">' +
							'<div class="group">' + 
								...
								'<div class="tools">' +
                                    ...
                                    '<input name="lt_trace_times_margin" type="text" id="lt_trace_times_margin" value="1" />' +
                                    // <span class="add"></span>' 
								'</div>' +
							'</div>' +
							'<div class="group">' + 
		...
		htmltext += '<span id="lt_sametime_html" style="display:none;">' +
						'<div class="group">' + 
							...
							'<div class="tools">' +
								// '<span class="reduce"></span>
                                '<input name="lt_trace_times_same" type="text" id="lt_trace_times_same" value="1" />' +
                                // <span class="add"></span>' +
							'</div>' +
						'</div>' +
					'</span>';			
        ...
        htmltext += '<span id="lt_difftime_html" style="display:none;">' + 
						'<div class="group">' + 
							...
							'<div class="tools">' +
								// '<span class="reduce"></span>
                                '<input name="lt_trace_diff" type="text" id="lt_trace_diff" value="1" />' +
                                // <span class="add"></span>' +
							'</div>' +
						...
						'<div class="group">' + 
							...
							'<div class="tools">' +
								// '<span class="reduce"></span>
                                '<input name="lt_trace_times_diff" type="text" id="lt_trace_times_diff" value="2" />' +
                                // <span class="add"></span>' +
							'</div>' +
						'</div>' +
					'</span>';
		htmltext += '<span>' + 
						'<div class="group">' + 
							...
							'<div class="tools">' +
								// '<span class="reduce"></span>
                                '<input name="lt_trace_count_input" type="text" id="lt_trace_count_input" value="10" />' +
                                // <span class="add"></span>' +
							'</div>' +
        ...
        var issueshtml = '<table cellspacing="0" cellpadding="0" id="lt_trace_issues_today" width="100%">' +
						  '<tr>' +
							'<th class="zh_th01"></th>' +
							'<th class="zh_th02">追号</th>' +
							'<th class="zh_th03">奖期</th>' +
							'<th class="zh_th04">倍数</th>' +
							'<th class="zh_th05">金额</th>' +
							'<th class="zh_th06">开奖时间</th>' +
						  '</tr>';
        ...
        $.each($.lt_issues.today,function(i,n){
            ...
            issueshtml += '<tr id="tr_trace_'+n.issue+'">' + 
							...
							'<td><span class="red">'+'<span id="lt_trace_money_'+n.issue+'">0.00</span></span></td>' + 
							'<td><span class="time_width">'+n.endtime+'</span></td>' + 
						 '</tr>';
        });
        ...
        $($.lt_id_data.id_tra_ok).click(function(){
            ...
            if( t_type == 'margin' ){//如果为利润率追号则首先检测是否支持
                var marmt = 0;
                var marmd = 0;
                var martype =0;//利润率支持情况，0:支持,1:玩法不支持，2:多玩法，3:多圆角模
            ...
            msg += lot_lang.am_s14.replace("[count]",ic);
             ...
             layer.open({
                ...
                yes:function(index){
                    ...
                    $.lt_trace_money = tm;
                    upTraceCount();
                    var aa = $('#lt_trace_hmoney').text();
                    $("#test_mm").text(aa) //BUG 修复用 没有其他用处  在 安卓 QQ浏览器弹窗不渲染的问题
                    layer.close(index)
                }
            })
