...
soyeScroll.init(function () {
    if (lock === 0) {
        lock = 1;
        $('.bet-recode-all').append('<li style="margin: auto;text-align: center;height: 2rem;display: block;" class="so-zzjz">正在加载...</li>');
        if (index == 1) {
            page++;
            restr = '';
...
$('#tabs .item').on('click', function () {
    ...
    if (index == 1) {
        ...
        page = 1;// 页数，从1开始
        restr = '';// 网页html缓存
        $('.tab_content').html('').ready(function () {
            getBetRecord(page, 2, 1); // 追号记录
        });
    } else {
        pageSize = 10;// 每页行数
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            if (type === 1 && page === 1) {
                ...
                $('.tab_content_' + status).append(restr);
                ...
                $('.tab_track_' + status).append(restr);
            }

            if (page > 1 && type === 1) {
                $.each(daysArr, function (j, m) {
                    if (dateDay.indexOf(j) >= 0) {
                        $('.slide_toggle').each(function (i, t) {
                            if (parseInt($(t).data('val'), 10) === parseInt(j, 10)) {
                                $.each(m, function (j, v) {
                                    var jsonStr = encodeURI('trace_record_dt01.html?data=' + JSON.stringify(v));
                                    var li_html = '<li class="bet_data" data-status="not_open">' +
                                        ...
                                        ' </a>' +
                                        '</li>';
                                    $('.slide_toggle ul').append(li_html);
                                });
                            }
                        });
                    } else {
